...
def cross_attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor):
    """
    Compute cross-attention between query and key/value tensors (no einsum).
    ...
    attn_scores = (query.unsqueeze(2) * keys).sum(dim=-1) / (D ** 0.5)  # (B, R, K)

    attn_weights = F.softmax(attn_scores, dim=-1)  # (B, R, K)

    attn_output = (attn_weights.unsqueeze(-1) * values).sum(dim=2)  # (B, R, D)

    return attn_output, attn_weights


def get_k_closest_embeddings(
    query: torch.Tensor,
    ...
    """
    Compute k closest candidate embeddings for batched queries,
    excluding any candidate that is identical to the query.

    Args:
        query (torch.Tensor): shape (B, R, D)
        candidates (torch.Tensor): shape (N, D)
        k (int): number of closest embeddings
        metric (str): 'cosine' or 'euclidean'
        eps (float): tolerance for equality check

    Returns:
        closest_embeddings: (B, R, k, D)
        indices: (B, R, k)
        scores: (B, R, k)
    """
    B, R, D = query.shape
    ...
    if metric == "cosine":
        query_norm = F.normalize(query, dim=-1)           # (B, R, D)
        candidates_norm = F.normalize(candidates, dim=-1) # (N, D)

      
        sims = torch.matmul(query_norm, candidates_norm.T)  # (B, R, N)

        diff = torch.abs(query.unsqueeze(2) - candidates.unsqueeze(0).unsqueeze(0))  # (B, R, N, D)
        identical_mask = (diff < eps).all(dim=-1)  # (B, R, N)
        sims = sims.masked_fill(identical_mask, float("-inf"))

        scores, indices = torch.topk(sims, k, dim=-1, largest=True)

    elif metric == "euclidean":
        diffs = query.unsqueeze(2) - candidates.unsqueeze(0).unsqueeze(0)  # (B, R, N, D)
        dists = torch.norm(diffs, dim=-1)  # (B, R, N)

        identical_mask = (torch.abs(dists) < eps)
        dists = dists.masked_fill(identical_mask, float("inf"))

        scores, indices = torch.topk(-dists, k, dim=-1, largest=True)

    else:
        raise ValueError("metric must be either 'cosine' or 'euclidean'")

    expanded_candidates = candidates.unsqueeze(0).unsqueeze(0).expand(B, R, N, D)
    closest_embeddings = torch.gather(
        expanded_candidates, 2,
        indices.unsqueeze(-1).expand(-1, -1, -1, D)
    )

    return closest_embeddings, indices, scores
